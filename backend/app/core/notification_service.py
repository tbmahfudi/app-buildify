...
class NotificationService:
    ...
    async def get_notification_config(
        ...
        if tenant_id:
            query = select(NotificationConfig).where(
                and_(NotificationConfig.tenant_id == tenant_id, NotificationConfig.is_active == True)
            )
        ...
        query = select(NotificationConfig).where(
            and_(NotificationConfig.tenant_id == None, NotificationConfig.is_active == True)
        )
        result = await db.execute(query)
    ...
    async def notify_password_changed(
        db: AsyncSession, user_email: str, user_id: Optional[str] = None, tenant_id: Optional[str] = None
    ) -> List[NotificationQueue]:
        """
...
async def notify_password_reset(
    db: AsyncSession, user_email: str, reset_link: str, user_id: Optional[str] = None, tenant_id: Optional[str] = None
) -> List[NotificationQueue]:
