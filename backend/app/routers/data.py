...
ENTITY_REGISTRY = {"companies": Company, "branches": Branch, "departments": Department, "users": User}
...
def get_record(entity: str, id: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    return DataResponse(id=str(record.id), data=model_to_dict(record))
...
def delete_record(
    entity: str, id: str, req: Request, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)
):
    """Delete a record"""
    model = get_model_class(entity)
...
def bulk_operation(
    ...
    for idx, record_data in enumerate(request.records):
        try:
            ...
            failed += 1
            errors.append({"index": idx, "record": record_data, "error": str(e)})
    ...
    try:
        ...
        create_audit_log(
            ...
            user=current_user,
            entity_type=entity,
            context_info={"total": len(request.records), "success": success, "failed": failed},
            request=req,
    ...
    return BulkOperationResponse(total=len(request.records), success=success, failed=failed, errors=errors)
