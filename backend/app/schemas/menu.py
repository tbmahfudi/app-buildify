...
class MenuItemBase(BaseModel):
    ...
    icon_color_primary: Optional[str] = Field(
        None, description="Primary color for duo-tone icons (e.g., '#3b82f6')", max_length=20
    )
    ...
    permission: Optional[str] = Field(
        None, description="Required permission (e.g., 'users:read:tenant')", max_length=200
    )
...
class MenuItemCreate(MenuItemBase):
    ...
    tenant_id: Optional[UUID | str] = Field(
        None, description="Tenant ID (null for system menus, auto-set for non-superusers)"
    )
