...
def check_seed_status():
    ...
    try:
        ...
        for tenant in tenants:
            ...
            sysadmin = (
                db.query(User)
                .filter(User.tenant_id == tenant.id, User.is_superuser == True, User.is_active == True)
                .first()
            )
            if sysadmin:
            ...
            workflow_count = (
                ...
                .filter(WorkflowDefinition.tenant_id == tenant.id, WorkflowDefinition.is_deleted == False)
                .count()
            )

            automation_count = (
                db.query(AutomationRule)
                .filter(AutomationRule.tenant_id == tenant.id, AutomationRule.is_deleted == False)
                .count()
            )

            lookup_count = (
                db.query(LookupConfiguration)
                .filter(LookupConfiguration.tenant_id == tenant.id, LookupConfiguration.is_deleted == False)
                .count()
            )
        ...
        if sysadmin_tenant:
            entity_count = (
                db.query(EntityDefinition)
                .filter(EntityDefinition.tenant_id == sysadmin_tenant.id, EntityDefinition.is_deleted == False)
                .count()
            )
