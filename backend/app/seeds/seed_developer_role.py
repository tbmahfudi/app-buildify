...
NOCODE_BUNDLE = [
    # Data & schema
    "data_model:read:tenant", "data_model:create:tenant",
    "data_model:update:tenant", "data_model:delete:tenant",
    "data_model:execute:tenant",
    # Lookups
    "lookups:read:tenant", "lookups:create:tenant",
    "lookups:update:tenant", "lookups:delete:tenant",
    # Builder / UI & pages
    "builder:design:tenant", "builder:publish:tenant",
    "builder:manage-menus:tenant", "builder:manage-permissions:tenant",
    "builder:pages:read:tenant", "builder:pages:create:tenant",
    "builder:pages:edit:tenant", "builder:pages:delete:tenant",
    # Reports & dashboards (create/read enough to build for subscribed modules)
    "reports:create:tenant", "reports:read:tenant",
    "reports:update:tenant", "reports:execute:tenant", "reports:export:tenant",
    "dashboards:read:tenant", "dashboards:create:tenant",
    "dashboards:update:tenant",
    # Business logic
    "workflows:read:tenant", "workflows:create:tenant",
    "workflows:update:tenant", "workflows:delete:tenant",
    "workflows:execute:tenant",
    "automations:read:tenant", "automations:create:tenant",
    "automations:update:tenant", "automations:delete:tenant",
...
NEW_PERMISSIONS = [
    ...
    },
    {
        ...
        "description": "View tenant notification configuration (operator-facing "
                       "System Management menu gate)",
...
SETTINGS_MANAGE_ROLES = [
    "tenant_admin", "company_manager", "company_admin",
    "branch_admin", "security_admin", "module_admin",
]
...
ADMIN_SETTINGS_GRANTS = [
    "settings:manage:tenant", "settings:read:tenant", "settings:update:tenant",
]
...
def _ensure_permission(db: Session, spec: dict) -> Permission:
    ...
    perm = Permission(
        ...
        resource=spec["resource"], action=spec["action"], scope=spec["scope"],
        category=spec["category"], is_system=spec["is_system"],
    )
    db.add(perm)
    db.flush()
...
def seed_developer_role():
    ...
    try:
        ...
        role = db.query(Role).filter(
            Role.code == "developer", Role.tenant_id.is_(None)
        ).first()
        if not role:
            role = Role(
                code="developer",
            ...
            role.name = "Developer"
            role.description = ("No-code / low-code builder: customize subscribed "
                                "modules (data models, pages, reports, workflows).")
            role.is_system = True
            role.is_active = True
        ...
        admin_roles = db.query(Role).filter(
            Role.code.in_(SETTINGS_MANAGE_ROLES)
        ).all()
        cnt = 0
        for r in admin_roles:
            for code in ADMIN_SETTINGS_GRANTS + OPERATOR_SYSTEM_GRANTS:
                if _grant(db, r, code):
                    cnt += 1
        db.commit()
        print(f"  ✓ Granted {cnt} settings/system perms across "
              f"{len(admin_roles)} admin role rows")
        ...
        operator_roles = db.query(Role).filter(
            Role.code.in_(OPERATOR_ROLES)
        ).all()
        ocnt = 0
        for r in operator_roles:
            for code in OPERATOR_SYSTEM_GRANTS:
                if _grant(db, r, code):
                    ocnt += 1
        db.commit()
        print(f"  ✓ Granted {ocnt} operator perms across "
        ...
        for r in ru:
            ...
            codes = {
                db.get(Permission, x.permission_id).code for x in rp
            }
