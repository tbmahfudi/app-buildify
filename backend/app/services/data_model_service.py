...
class DataModelService:
    ...
    async def create_entity(self, entity_data: EntityDefinitionCreate, is_platform_level: bool = False):
        ...
        if is_platform_level and not self.current_user.is_superuser:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Only superusers can create platform-level entities"
            )
        ...
        if is_platform_level:
            ...
            )  # tenant-scope-ok (platform-level None check — or_() intentional cross-scope)
        else:
        ...
        existing = (
            self.db.query(EntityDefinition)
            .filter(existing_filter, EntityDefinition.name == entity_data.name, EntityDefinition.is_deleted == False)
            .first()
        )
        ...
        if entity_data.module_id:
            module = self.db.query(NocodeModule).filter(NocodeModule.id == entity_data.module_id).first()
        ...
        entity = EntityDefinition(
            **entity_dict, tenant_id=target_tenant_id, created_by=self.current_user.id, updated_by=self.current_user.id
        )
    ...
    async def list_entities(
        ...
        query = self.db.query(EntityDefinition).filter(EntityDefinition.is_deleted == False)
        if include_platform:
            _tid = self.current_user.tenant_id  # tenant-scope-ok (or_() platform-include setup)
            query = query.filter(
                or_(
    ...
    async def get_entity(self, entity_id: UUID, include_platform: bool = True):
        ...
        entity_q = self.db.query(EntityDefinition).filter(
            EntityDefinition.id == entity_id, EntityDefinition.is_deleted == False
        )
        if include_platform:
            _tid = self.current_user.tenant_id  # tenant-scope-ok (or_() platform-include setup)
    ...
    async def create_field(self, entity_id: UUID, field_data: FieldDefinitionCreate):
        ...
        if existing_active:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail=f"Field with name '{field_data.name}' already exists"
            )
    ...
    async def list_fields(self, entity_id: UUID, include_deleted: bool = False):
        ...
        query = self.db.query(FieldDefinition).filter(FieldDefinition.entity_id == entity_id)
    ...
    async def delete_field(self, entity_id: UUID, field_id: UUID):
        ...
        if field.is_system:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Cannot delete system field")
    ...
    async def restore_field(self, entity_id: UUID, field_id: UUID):
        ...
        if not field:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Deleted field not found")
    ...
    async def list_deleted_fields(self, entity_id: UUID):
        ...
        deleted_fields = (
            self.db.query(FieldDefinition)
            .filter(FieldDefinition.entity_id == entity_id, FieldDefinition.is_deleted == True)
            .order_by(FieldDefinition.updated_at.desc())
            .all()
        )
    ...
    async def clone_entity(self, entity_id: UUID, new_name: str = None, new_label: str = None):
        ...
        if existing:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail=f"Entity with name '{new_name}' already exists"
            )
        ...
        source_fields = (
            self.db.query(FieldDefinition)
            .filter(FieldDefinition.entity_id == source_entity.id, FieldDefinition.is_deleted == False)
            .all()
        )
    ...
    async def list_relationships(self, entity_id: Optional[UUID] = None):
        """List relationships"""
        query = self.db.query(RelationshipDefinition).filter(RelationshipDefinition.is_deleted == False)
    ...
    async def delete_relationship(self, relationship_id: UUID):
        ...
        rel_q = self.db.query(RelationshipDefinition).filter(RelationshipDefinition.id == relationship_id)
    ...
    async def preview_migration(self, entity_id: UUID):
        ...
        entity = (
            self.db.query(EntityDefinition)
            .filter(EntityDefinition.id == entity_id, EntityDefinition.is_deleted == False)
            .first()
        )
        ...
        if entity.tenant_id and entity.tenant_id != self.tenant_id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="You don't have permission to preview this entity"
            )
    ...
    async def generate_migration(self, entity_id: UUID, commit_message: str = None):
        ...
        existing_pending = (
            self.db.query(EntityMigration)
            .filter(EntityMigration.entity_id == entity_id, EntityMigration.status == "pending")
            .first()
        )
    ...
    async def publish_entity(self, entity_id: UUID, commit_message: str = None):
        ...
        if entity.tenant_id and entity.tenant_id != self.tenant_id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="You don't have permission to publish this entity"
            )
        ...
        try:
            ...
            try:
                ...
                metadata_sync.auto_generate_metadata(entity_definition=entity, created_by=str(self.current_user.id))
            ...
            try:
                ...
                parent_menu = MenuService.get_or_create_nocode_parent(
                    self.db, entity.tenant_id, str(self.current_user.id)
                )
                ...
                # Create menu item
                MenuService.create_menu_item(db=self.db, user=self.current_user, menu_data=menu_data)
    ...
    async def list_migrations(self, entity_id: UUID):
        ...
        migrations = (
            self.db.query(EntityMigration)
            .filter(EntityMigration.entity_id == entity_id)
            .order_by(EntityMigration.created_at.desc())
            .all()
        )
    ...
    async def execute_migration(self, migration_id: UUID):
        ...
        migration = self.db.query(EntityMigration).filter(EntityMigration.id == migration_id).first()
        ...
        if migration.tenant_id and migration.tenant_id != self.tenant_id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="You don't have permission to execute this migration"
            )
        ...
        entity = self.db.query(EntityDefinition).filter(EntityDefinition.id == migration.entity_id).first()

        if not entity:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"Entity not found for migration {migration_id}"
            )
        ...
        try:
            ...
            try:
                ...
                MenuService.get_or_create_nocode_parent(self.db, entity.tenant_id, str(self.current_user.id))
    ...
    async def rollback_migration(self, migration_id: UUID):
        ...
        if migration.tenant_id and migration.tenant_id != self.tenant_id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="You don't have permission to rollback this migration"
            )

        if migration.status != "completed":
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Can only rollback completed migrations"
            )

        if not migration.down_script:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="No rollback script available for this migration"
            )
    ...
    async def _estimate_impact(self, entity: EntityDefinition) -> dict:
        ...
        if not table_exists:
            return {"risk_level": "low", "affected_records": 0, "breaking_changes": [], "warnings": []}

        # Check for existing data
        try:
            result = self.db.execute(text(f"SELECT COUNT(*) FROM {entity.table_name}"))
        ...
        if changes["operation"] == "ALTER":
            ...
            if change_details.get("removed_columns"):
                breaking_changes.append(f"Dropping columns: {', '.join(change_details['removed_columns'])}")

            # Type changes can be risky
            if change_details.get("modified_columns"):
                for mod in change_details["modified_columns"]:
                    warnings.append(f"Column {mod['name']} type changing from {mod['from_type']} to {mod['to_type']}")
