...
class PayrollService:
    ...
    def __init__(self, db: Session, current_user):
        ...
        try:
            self.employee_service = registry.get_service(
                module_name="hr", service_name="EmployeeService", db=db, current_user=current_user
            )
    ...
    def calculate_payroll(self, employee_id: str, month: str, year: int) -> dict:
        ...
        if not self.employee_service:
            raise ServiceNotFoundError(
                "HR module is required but not available. " "Please ensure HR module is installed and active."
            )

        # Check permission
        if not self._has_permission("payroll:calculate"):
            raise PermissionError("No permission to calculate payroll. Required: payroll:calculate")
        ...
        if not profile:
            raise ValueError(
                f"No payroll profile found for employee {employee['name']}. " "Please create payroll profile first."
            )
        ...
        return {
            ...
            "payment_details": {"bank_account": profile["bank_account"], "payment_method": profile["payment_method"]},
        }

    def list_payroll_for_department(self, department_id: str, month: str, year: int) -> List[dict]:
        """
        ...
        employees = self.employee_service.list_employees(department_id=department_id, status="active")

        payroll_results = []

        for employee in employees:
            try:
                payroll = self.calculate_payroll(employee_id=employee["id"], month=month, year=year)
                payroll_results.append(payroll)
            except ValueError as e:
                # Skip employees without payroll profiles
                payroll_results.append({"employee": employee, "error": str(e), "status": "error"})
    ...
    def _calculate_deductions(self, gross_salary: Decimal, profile: dict) -> dict:
        ...
        return {"tax": tax, "insurance": insurance, "retirement": retirement, "other": other, "total": total}
