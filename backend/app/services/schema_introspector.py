...
class SchemaIntrospector:
    ...
    async def list_database_objects(self, schema: str = "public") -> Dict[str, List[Dict]]:
        ...
        return {
            "tables": [
                {"name": table, "type": "table", "schema": schema, "comment": self._get_table_comment(table, schema)}
                for table in tables
                if not self._is_system_table(table)
            ],
            "views": [
                {"name": view, "type": "view", "schema": schema, "definition": self._get_view_definition(view, schema)}
                for view in views
    ...
    def _get_view_definition(self, view_name: str, schema: str) -> Optional[str]:
        ...
        """
        result = self.db.execute(text(query), {"view_name": view_name, "schema": schema}).fetchone()
    ...
    def _get_table_comment(self, table_name: str, schema: str) -> Optional[str]:
        ...
        """
        result = self.db.execute(text(query), {"table": table_name, "schema": schema}).fetchone()
    ...
    async def introspect_object(self, object_name: str, object_type: str, schema: str = "public") -> Dict[str, Any]:
        """
    ...
    async def _introspect_table(self, table_name: str, schema: str) -> Dict[str, Any]:
        ...
        entity_def = {
            ...
            # Generate fields
            "fields": self._generate_field_definitions(columns, pk_columns, fk_constraints, unique_constraints),
    ...
    async def _introspect_view(self, view_name: str, object_type: str, schema: str) -> Dict[str, Any]:
        """
    ...
    def _generate_field_definitions(
        self, columns: List[Dict], pk_columns: List[str], fk_constraints: List[Dict], unique_constraints: List[Dict]
    ) -> List[Dict]:
        """
        ...
        for col in columns:
            ...
            if col_name in ["created_at", "updated_at", "created_by", "updated_by", "deleted_at", "is_deleted"]:
                continue
    ...
    def _generate_relationships(self, fk_constraints: List[Dict], source_table: str) -> List[Dict]:
        """
    ...
    def _generate_index_definitions(self, indexes: List[Dict], pk_columns: List[str]) -> List[Dict]:
