...
class TemplatePackageService:
    ...
    async def export_entity_template(self, entity_id: UUID, include_fields: bool = True) -> dict:
        """Export an entity template as JSON."""
        entity = self.db.query(EntityDefinition).filter(EntityDefinition.id == entity_id).first()
        ...
        if include_fields:
            fields = (
                ...
                .filter(FieldDefinition.entity_id == entity_id, FieldDefinition.is_deleted == False)
    ...
    async def create_package(
        ...
        if not self.current_user.is_superuser:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Only superusers can create template packages"
            )
        ...
        existing = self.db.query(TemplatePackage).filter(TemplatePackage.code == code).first()

        if existing:
            raise HTTPException(status_code=400, detail=f"Package with code '{code}' already exists")
            ...
            package_data=json.dumps(
                {
                    ...
                    "metadata": {"template_count": len(package_templates), "created_at": datetime.utcnow().isoformat()},
                },
                indent=2,
            ),
    ...
    async def export_package_as_zip(self, package_id: UUID) -> bytes:
        ...
        package = self.db.query(TemplatePackage).filter(TemplatePackage.id == package_id).first()
...
"""
    ...
    async def import_entity_template(
        self, template_data: dict, target_tenant_id: UUID = None, make_platform_level: bool = False
    ) -> EntityDefinition:
        ...
        existing = (
            ...
            .filter(EntityDefinition.name == template["name"], EntityDefinition.is_deleted == False)
            .first()
        )
        ...
        if make_platform_level:
            if not self.current_user.is_superuser:
                raise HTTPException(status_code=403, detail="Only superusers can import as platform-level templates")
            tenant_id = None
        else:
    ...
    async def import_package(
        self, package_data: dict, target_tenant_id: UUID = None, make_platform_level: bool = False
    ) -> dict:
        ...
        for template_data in templates:
            try:
                if template_data["type"] == "entity":
                    entity = await self.import_entity_template(template_data, target_tenant_id, make_platform_level)
                    imported_templates.append({"type": "entity", "id": str(entity.id), "name": entity.name})
                # Add logic for other template types...

            except Exception as e:
                errors.append({"template": template_data.get("template", {}).get("name", "Unknown"), "error": str(e)})
    ...
    async def import_from_zip(
        self, zip_file: UploadFile, target_tenant_id: UUID = None, make_platform_level: bool = False
    ) -> dict:
        ...
        try:
            ...
            with zipfile.ZipFile(zip_buffer, "r") as zip_ref:
                ...
                # Import package
                result = await self.import_package(package_data, target_tenant_id, make_platform_level)

                return result

        except Exception as e:
            raise HTTPException(status_code=400, detail=f"Failed to import package: {str(e)}")

    # ==================== List Methods ====================

    async def list_packages(self, category_id: UUID = None, is_verified: bool = None) -> List[TemplatePackage]:
        """List available template packages."""
        query = self.db.query(TemplatePackage).filter(
            TemplatePackage.is_active == True, TemplatePackage.is_published == True
        )
