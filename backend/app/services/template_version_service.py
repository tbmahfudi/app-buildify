...
class TemplateVersionService:
    ...
    async def create_version(
        ...
        if not self.current_user.is_superuser:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Only superusers can create template versions"
            )
        ...
        latest_version = (
            ...
            .filter(TemplateVersion.template_type == template_type, TemplateVersion.template_id == template_id)
    ...
    async def list_versions(self, template_type: str, template_id: UUID) -> List[TemplateVersion]:
    ...
    async def rollback_to_version(self, template_type: str, template_id: UUID, version_number: int):
        """Rollback a template to a previous version."""
        # Only superusers can rollback
        if not self.current_user.is_superuser:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only superusers can rollback templates")
    ...
    async def compare_versions(self, template_type: str, template_id: UUID, from_version: int, to_version: int):
    ...
    async def _get_template_snapshot(self, template_type: str, template_id: UUID) -> dict:
        ...
        if template_type == "entity":
            entity = self.db.query(EntityDefinition).filter(EntityDefinition.id == template_id).first()
            ...
            fields = (
                ...
                .filter(FieldDefinition.entity_id == template_id, FieldDefinition.is_deleted == False)
                .all()
            )

            return {"entity": self._model_to_dict(entity), "fields": [self._model_to_dict(f) for f in fields]}

        elif template_type == "workflow":
            workflow = self.db.query(WorkflowDefinition).filter(WorkflowDefinition.id == template_id).first()
            ...
            states = (
                ...
                .filter(WorkflowState.workflow_id == template_id, WorkflowState.is_deleted == False)
                .all()
            )

            transitions = (
                self.db.query(WorkflowTransition)
                .filter(WorkflowTransition.workflow_id == template_id, WorkflowTransition.is_deleted == False)
                .all()
            )
            ...
            rule = self.db.query(AutomationRule).filter(AutomationRule.id == template_id).first()

            if not rule:
                raise HTTPException(status_code=404, detail="Automation rule not found")

            return {"rule": self._model_to_dict(rule)}

        elif template_type == "lookup":
            lookup = self.db.query(LookupConfiguration).filter(LookupConfiguration.id == template_id).first()

            if not lookup:
                raise HTTPException(status_code=404, detail="Lookup not found")

            return {"lookup": self._model_to_dict(lookup)}
        ...
        for key in from_snapshot.keys():
            if key in to_snapshot:
                if from_snapshot[key] != to_snapshot[key]:
                    changes.append(
                        {"type": "modified", "field": key, "from": from_snapshot[key], "to": to_snapshot[key]}
                    )
            else:
                changes.append({"type": "removed", "field": key, "from": from_snapshot[key]})

        for key in to_snapshot.keys():
            if key not in from_snapshot:
                changes.append({"type": "added", "field": key, "to": to_snapshot[key]})
