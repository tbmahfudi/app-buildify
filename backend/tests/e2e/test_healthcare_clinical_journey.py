...
def hc():
    ...
    Yields an object with `.get/.post(path, ...)` that prefix the healthcare API and carry
    the staff token + X-Branch-ID. Skips (never fails) when the stack/seed is not present.
    """
    base = _resolve_hc_base()
    token = _login_token()
    branch = DEFAULT_BRANCH
    headers = {"Authorization": f"Bearer {token}", "X-Branch-ID": branch}
    client = httpx.Client(base_url=f"{base}{HC_PREFIX}", headers=headers, timeout=TIMEOUT)

    # Confirm this staff user actually has access to this branch, else the journey is moot.
    probe = client.get(f"/branches/{branch}/encounters")
...
def test_clinical_journey_registration_to_coding(hc):
    ...
    r = hc.post(f"/branches/{b}/queue-tickets/{ticket_id}/call")
    assert r.status_code == 200, r.text
    called = r.json()
