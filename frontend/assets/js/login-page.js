...
export function renderRequestReset(container) {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    setRequestLoadingState(wrapper, true);
    ...
    try {
      ...
      showEmailSentState(wrapper, email);
    } catch (_err) {
      errEl.textContent = "Network error — please try again.";
      errEl.classList.remove("hidden");
      setRequestLoadingState(wrapper, false);
    }
  });
}
...
function setRequestLoadingState(wrapper, isLoading) {
 ...
 * Replaces the entire form with a confirmation message.
 * Does NOT reveal whether the email address is registered.
 */
function showEmailSentState(wrapper, email) {
