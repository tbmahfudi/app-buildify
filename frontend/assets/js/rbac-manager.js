...
class RBACManager {
  ...
  async loadTabData(tabId) {
    switch (tabId) {
      ...
      case 'users':
        ...
        break;
      case 'user-roles':
        await this.loadAssignmentHelperView('user-roles');
        break;
      case 'role-permissions':
        await this.loadAssignmentHelperView('role-permissions');
        break;
      case 'matrix':
        await this.loadAssignmentHelperView('matrix');
        break;
    }
  }
  ...
  async loadAssignmentHelperView(view) {
    try {
      showLoading();

      // Initialize the helper if not already done
      if (!permissionAssignmentHelper.containerId) {
        await permissionAssignmentHelper.initialize(`content-${view}`);
      }
      ...
      console.log(`✓ Assignment Helper view '${view}' loaded successfully`);

    } catch (error) {
      console.error(`Error loading Assignment Helper view '${view}':`, error);
      ...
      if (container) {
        container.innerHTML = `
          <div class="bg-white rounded-xl shadow p-12 text-center">
            ...
            <h3 class="text-lg font-semibold text-gray-900 mb-2">Failed to Load View</h3>
            <p class="text-sm text-gray-600 mb-4">${error.message || 'An unexpected error occurred'}</p>
            <button onclick="rbacManager.loadAssignmentHelperView('${view}')"
