 ...
 *
 * Note: This is the same security administration interface as auth-policies,
 * accessible from both Administration > Auth Policies and System Settings > Security
 */
...
document.addEventListener('route:loaded', async (event) => {
  ...
  // Initialize SecurityAdmin when settings-security route is loaded
  if (route === 'settings-security') {
    console.log('Initializing Security Administration (from Settings)...');
    try {
      ...
      console.log('Security Administration initialized successfully (from Settings)');
