...
- **⛔ D7 itself is now blocked on [ADR-012](../architecture/adr-012-module-saas-tenancy-and-end-user-rbac.md)
  (Proposed).** D7 step 2 says the backfill assigns the `patient` role + `patients` group,
  but ADR-HC-010 moved every patient to the shared **SAAS** tenant and those RBAC artifacts
  exist **only in the old per-clinic tenants** — so the step is unsatisfiable as written.
  ADR-012 resolves it generically (module-declared tenancy + end-user RBAC provisioned into
  the shared tenant) rather than hardcoding a healthcare answer, because other modules may
  adopt SaaS mode too. **Chain: ADR-012 → D7 backfill + `/patient/claim-account` (+ GH#693)
  → S6b (GH#692).**
