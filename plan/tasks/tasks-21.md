...
| T-21.1.3 | Implement `flex-sidebar.js` (collapse threshold; subscribes to FlexResponsive.breakpointChange) | C3 | T-21.1.1 | 8 | [epic-15 §15.1.1 — FlexSidebar](../epics/epic-15-flex-component-library.md) | DONE — uses `getResponsive().onResize()` (continuous) instead of breakpointChange (discrete) since collapse threshold doesn't align with breakpoint boundaries; supports `side: left|right`; emits `collapsed-change`; 13/13 inline tests pass |
